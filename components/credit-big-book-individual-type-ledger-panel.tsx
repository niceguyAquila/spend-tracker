...
import type { BigBookMonthlyCurrencyRow, CreditBookEntry, CreditBookLedgerType } from "@/lib/types";
...
export function CreditBigBookIndividualTypeLedgerPanel({ types }: Props) {
  ...
  const [entries, setEntries] = useState<CreditBookEntry[]>([]);
  ...
  const loadTypeLedger = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/credit-big-book/type-ledger?${params.toString()}`);
