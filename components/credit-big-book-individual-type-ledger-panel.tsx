...
import type { CreditBookEntry, CreditBookLedgerType } from "@/lib/types";
import {
...
export function CreditBigBookIndividualTypeLedgerPanel({ types, entries }: Props) {
  ...
  const [directionFilter, setDirectionFilter] = useState<Array<"credit" | "debt">>([]);
  ...
  const visibleEntries = useMemo(() => {
    return filterCreditIndividualTypeEntries(selectedTypeEntries, {
  ...
  const monthlyRows = useMemo(() => {
    return buildCreditIndividualTypeMonthlySummary(selectedTypeEntries, selectedYear);
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-4">
          ...
          <label className="text-sm text-muted">
            ...
            <SearchableMultiSelect
              ...
              onChange={(next) => setDirectionFilter(next as Array<"credit" | "debt">)}
            ...
            <tbody>
              {pagedVisibleEntries.map((entry) => (
                <tr key={entry.id} className="border-b">
                  ...
                  <td className={`px-3 py-2 ${getAmountColorClass(entry.entry_direction === "debt" ? -entry.amount : entry.amount)}`}>
