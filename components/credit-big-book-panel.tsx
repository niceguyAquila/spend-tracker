...
export function CreditBigBookPanel({
  ...
  const [pendingDeleteEntry, setPendingDeleteEntry] = useState<CreditBookEntry | null>(null);
  ...
  const [manageAttachmentsEntry, setManageAttachmentsEntry] = useState<CreditBookEntry | null>(null);
  ...
  const [entries, setEntries] = useState<CreditBookEntry[]>(initialEntries);
  ...
  // `getCreditBookActorCurrencyMetrics`), not just the currently rendered page.
  ...
  const applyMetricDelta = useCallback(
    (
      ...
      setActorCurrencyMetrics((prev) => {
        ...
        const inserted: CreditBookActorCurrencyMetrics = {
          actor_id: actorId,
  ...
  async function createEntry() {
    ...
    setMessage(null);
    try {
      const response = await secureFetch("/api/credit-big-book/entries", {
        method: "POST",
      ...
      if (createAttachmentFiles.length > 0) {
        for (const file of createAttachmentFiles) {
          ...
          const uploadResponse = await secureFetch("/api/credit-big-book/attachments", {
            method: "POST",
            body: formData
          });
  ...
  async function deleteEntry() {
    ...
    setMessage(null);
    try {
      const response = await secureFetch(`/api/credit-big-book/entries?id=${pendingDeleteEntry.id}`, { method: "DELETE" });
      if (handleUnauthorizedResponse(response)) return;
      const data = await response.json();
      if (!response.ok) {
  ...
  async function importEntries() {
    ...
    try {
      ...
      const response = await secureFetch("/api/credit-big-book/import", {
        method: "POST",
        body: formData
      });
      if (handleUnauthorizedResponse(response)) return;
  ...
  function startEditEntry(row: CreditBookEntry) {
    setOpenActionMenu(null);
  ...
  async function uploadAttachments() {
    ...
    try {
      for (const file of files) {
        ...
        formData.append("file", file);
        const response = await secureFetch("/api/credit-big-book/attachments", {
          method: "POST",
          body: formData
        });
        if (handleUnauthorizedResponse(response)) return;
  ...
  async function viewAttachment(attachmentId: string) {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/credit-big-book/attachments/view?id=${attachmentId}`);
      if (handleUnauthorizedResponse(response)) return;
      const data = await response.json();
  ...
  async function deleteAttachment() {
    ...
    setError(null);
    try {
      const response = await secureFetch(`/api/credit-big-book/attachments?id=${pendingDeleteAttachmentId}`, {
        method: "DELETE"
      });
      if (handleUnauthorizedResponse(response)) return;
      const data = await response.json();
  ...
  function openManageAttachments(row: CreditBookEntry) {
    setOpenActionMenu(null);
            ...
            <label className="text-sm">
              ...
              <select
                ...
                onChange={(event) =>
                  setEntryForm((prev) => ({
                    ...prev,
                    entry_direction: event.target.value as "debt" | "credit"
                  }))
                }
              >
      ...
      <ConfirmDialog
        ...
        description={
          createAttachmentFiles.length
            ...
            : "This will create a new credit/debt record in the Credit Big Book."
        }
