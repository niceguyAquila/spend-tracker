...
export function BigBookExchangeHelperPanel() {
  ...
  const [baseCurrency, setBaseCurrency] = useState<BigBookCurrencyCode>("USDT");
  const [quoteCurrency, setQuoteCurrency] = useState<BigBookCurrencyCode>("MYR");
  ...
  return (
    <section className="card">
      ...
      <p className="mt-1 text-sm text-slate-600">Get a live quote from Coinbase public rates for Big Book currencies.</p>
        ...
        <div className="grid grid-cols-1 gap-3 md:max-w-xl md:grid-cols-2">
          <label className="text-sm">
            ...
            <select
              className="field mt-1"
              value={baseCurrency}
              onChange={(event) => setBaseCurrency(event.target.value as BigBookCurrencyCode)}
            >
          ...
          <label className="text-sm">
            ...
            <select
              className="field mt-1"
              value={quoteCurrency}
              onChange={(event) => setQuoteCurrency(event.target.value as BigBookCurrencyCode)}
            >
              {CURRENCIES.map((currency) => (
                <option key={currency} value={currency}>
                  {currency}
