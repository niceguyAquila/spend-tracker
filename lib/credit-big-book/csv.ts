import { creditBookCurrencySchema, creditBookEntryDirectionSchema } from "@/lib/validation/credit-big-book";
...
export type ParsedCreditBigBookCsvRow = {
  entry_date: string;
...
export type ParseCreditBigBookCsvResult = {
  rows: ParsedCreditBigBookCsvRow[];
  errors: string[];
};
...
export function parseCreditBigBookCsv(content: string): ParseCreditBigBookCsvResult {
  ...
  const parsedRows: ParsedCreditBigBookCsvRow[] = [];
  ...
  for (let index = 0; index < dataRows.length; index += 1) {
    ...
    const directionParsed = creditBookEntryDirectionSchema.safeParse(entryDirectionRaw.toLowerCase());
    if (!directionParsed.success) {
      errors.push(`Row ${lineNumber}: entry_direction must be 'credit' or 'debt'.`);
      continue;
    }
    ...
    const currencyParsed = creditBookCurrencySchema.safeParse(currencyRaw);
