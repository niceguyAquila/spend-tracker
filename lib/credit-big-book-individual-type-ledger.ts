import type { CreditBookEntry, BigBookMonthlyCurrencyRow } from "@/lib/types";

export type CreditIndividualTypeLedgerFilters = {
  dateFrom?: string;
  dateTo?: string;
  currencyCode?: string[];
...
export function filterCreditIndividualTypeEntries(
  entries: CreditBookEntry[],
  filters: CreditIndividualTypeLedgerFilters
): CreditBookEntry[] {
...
export function buildCreditIndividualTypeMonthlySummary(
  entries: CreditBookEntry[],
  year: number
  ...
  for (const entry of entries) {
    ...
    const signedAmount = entry.entry_direction === "debt" ? -Math.abs(entry.amount) : Math.abs(entry.amount);
