...
import { expenseTypeCreateSchema, expenseTypeUpdateSchema } from "@/lib/validation/expense";
...
export async function POST(request: Request) {
  ...
  const parsed = expenseTypeCreateSchema.safeParse(body);
  if (!parsed.success) {
...
export async function PATCH(request: Request) {
  ...
  const body = await request.json();
  const parsed = expenseTypeUpdateSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json({ error: parsed.error.flatten() }, { status: 400 });
  }
