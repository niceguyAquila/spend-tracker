...
import {
  bigBookActionByCreateSchema,
  bigBookActionByUpdateSchema
} from "@/lib/validation/big-book";

const ENTITY_LABEL = "Action By";
...
export async function POST(request: Request) {
  ...
  const parsed = bigBookActionByCreateSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json({ error: parsed.error.flatten() }, { status: 400 });
  }

  const supabase = await createClient();
  const { data: existingActionBy, error: existingError } = await supabase
    .from("business_ledger_action_by")
  ...
  const conflict = findLabelConflict(existingActionBy, parsed.data, ENTITY_LABEL);
  if (conflict) {
  ...
  const sortOrder =
    typeof parsed.data.sort_order === "number"
      ...
      : nextSortOrder(existingActionBy);
...
export async function PATCH(request: Request) {
  ...
  const body = await request.json();
  const parsed = bigBookActionByUpdateSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json({ error: parsed.error.flatten() }, { status: 400 });
  }
  ...
  const supabase = await createClient();
  const { error } = await supabase.from("business_ledger_action_by").update(payload).eq("id", id);
  if (error) {
    return NextResponse.json({ error: describeWriteError(error, ENTITY_LABEL) }, { status: 400 });
  }
...
export async function DELETE(request: Request) {
  ...
  if (!id) {
    return NextResponse.json({ error: "Action By ID is required." }, { status: 400 });
  }
  ...
  if (!data) {
    return NextResponse.json({ error: "Action By not found." }, { status: 404 });
  }
