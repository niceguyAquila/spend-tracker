...
import { creditBookActorUpdateSchema } from "@/lib/validation/credit-big-book";
...
export async function GET() {
  ...
  const [actorsRes, usersRes] = await Promise.all([
    supabase.from("credit_book_actors").select("id, actor_code, display_name, user_id").order("actor_code"),
    supabase
      .from("allowed_users")
...
export async function PATCH(request: Request) {
  ...
  const parsed = creditBookActorUpdateSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const supabase = await createClient();
  const { error } = await supabase.from("credit_book_actors").update(payload).eq("id", id);
  if (error) {
