...
import {
  creditBookAttachmentCreateSchema,
  creditBookAttachmentDeleteSchema
...
export async function POST(request: Request) {
  ...
  const createInput = creditBookAttachmentCreateSchema.safeParse({
...
export async function DELETE(request: Request) {
  ...
  const parsed = creditBookAttachmentDeleteSchema.safeParse({ id: searchParams.get("id") });
  if (!parsed.success) {
  ...
  const { error } = await supabase.from("credit_ledger_attachments").delete().eq("id", row.id);
  if (error) {
    return NextResponse.json({ error: error.message }, { status: 400 });
  }
