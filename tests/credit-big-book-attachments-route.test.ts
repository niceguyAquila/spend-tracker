...
describe("credit big book attachments routes", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("uploads attachment and stores metadata", async () => {
    const { POST } = await import("@/app/api/credit-big-book/attachments/route");
    ...
    const request = new Request("https://app.localhost/api/credit-big-book/attachments", {
      method: "POST",
      body: formData
    });
