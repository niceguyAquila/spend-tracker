...
describe("big book exchange rate route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns conversion quote for valid request", async () => {
    ...
    const { GET } = await import("@/app/api/big-book/exchange-rate/route");
    const request = new Request(
      "https://app.localhost/api/big-book/exchange-rate?amount=1000&base_currency=USDT&quote_currency=MYR"
    );
  ...
  it("returns 400 for invalid query parameters", async () => {
    const { GET } = await import("@/app/api/big-book/exchange-rate/route");
    const request = new Request(
      "https://app.localhost/api/big-book/exchange-rate?amount=0&base_currency=INVALID&quote_currency=MYR"
    );
