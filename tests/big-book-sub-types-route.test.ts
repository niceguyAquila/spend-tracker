...
vi.mock("@/lib/supabase/server", () => ({
  createClient: vi.fn(async () => ({
    from: vi.fn((table: string) => {
      if (table === "business_ledger_sub_types") {
        return {
...
describe("big book sub-types route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("deletes a sub-type via DELETE", async () => {
    const { DELETE } = await import("@/app/api/big-book/sub-types/route");
    const request = new Request(
      "https://app.localhost/api/big-book/sub-types?id=55555555-5555-4555-8555-555555555555",
      { method: "DELETE" }
    );
